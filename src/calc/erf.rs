...
impl<T: ag::Float> ag::op::Op<T> for Erf {
    fn compute(&self, ctx: &mut ag::op::ComputeContext<T>) {
        ...
        let output = z.mapv(move |x| T::from(_erf(num::NumCast::from(x).unwrap())).unwrap());
...
impl<T: ag::Float> ag::op::Op<T> for ErfInv {
    fn compute(&self, ctx: &mut ag::op::ComputeContext<T>) {
        let z: &ag::NdArrayView<_> = &ctx.input(0);
        let output = z.mapv(move |x| T::from(_erf_inv(num::NumCast::from(x).unwrap())).unwrap());
        ctx.append_output(output);
    }
...
impl<T: ag::Float> ag::op::Op<T> for ErfC {
    fn compute(&self, ctx: &mut ag::op::ComputeContext<T>) {
        let z: &ag::NdArrayView<_> = &ctx.input(0);
        let output = z.mapv(move |x| T::from(_erfc(num::NumCast::from(x).unwrap())).unwrap());
        ctx.append_output(output);
    }
...
impl<T: ag::Float> ag::op::Op<T> for ErfCInv {
    fn compute(&self, ctx: &mut ag::op::ComputeContext<T>) {
        let z: &ag::NdArrayView<_> = &ctx.input(0);
        let output = z.mapv(move |x| T::from(_erfc_inv(num::NumCast::from(x).unwrap())).unwrap());
        ctx.append_output(output);
    }
...
mod tests {
    ...
    pub fn test_erf() {
        ag::with(|g: &mut ag::Graph<f64>| {
            let z: ag::Tensor<f64> = g.ones(&[5usize, 5usize]);
            let y: ag::Tensor<f64> = erf(g, &z);
            let x: ag::Tensor<f64> = erfinv(g, &y);

            let output = x.eval(&[]).expect("Failed to perform erf.");
            assert!(output.all_close(&ag::ndarray_ext::ones(&[5, 5]), 5e-10));
        });
    }
}

fn _polynomial(z: f64, coeff: &[f64]) -> f64 {
    let n = coeff.len();
    if n == 0 {
        return 0.0;
    }
...
fn erf_impl(z: f64, inv: bool) -> f64 {
    ...
    let result = if z < 0.5 {
        if z < 1e-10 {
            ...
            z * 1.125 + z * _polynomial(z, ERF_IMPL_AN) / _polynomial(z, ERF_IMPL_AD)
        }
    } else if z < 110.0 {
        let (r, b) = if z < 0.75 {
            (
                _polynomial(z - 0.5, ERF_IMPL_BN) / _polynomial(z - 0.5, ERF_IMPL_BD),
                0.3440242112,
            )
        } else if z < 1.25 {
            (
                _polynomial(z - 0.75, ERF_IMPL_CN) / _polynomial(z - 0.75, ERF_IMPL_CD),
                0.419990927,
            )
        } else if z < 2.25 {
            (
                _polynomial(z - 1.25, ERF_IMPL_DN) / _polynomial(z - 1.25, ERF_IMPL_DD),
                0.4898625016,
            )
        } else if z < 3.5 {
            (
                _polynomial(z - 2.25, ERF_IMPL_EN) / _polynomial(z - 2.25, ERF_IMPL_ED),
                0.5317370892,
            )
        } else if z < 5.25 {
            (
                _polynomial(z - 3.5, ERF_IMPL_FN) / _polynomial(z - 3.5, ERF_IMPL_FD),
                0.5489973426,
            )
        } else if z < 8.0 {
            (
                _polynomial(z - 5.25, ERF_IMPL_GN) / _polynomial(z - 5.25, ERF_IMPL_GD),
                0.5571740866,
            )
        } else if z < 11.5 {
            (
                _polynomial(z - 8.0, ERF_IMPL_HN) / _polynomial(z - 8.0, ERF_IMPL_HD),
                0.5609807968,
            )
        } else if z < 17.0 {
            (
                _polynomial(z - 11.5, ERF_IMPL_IN) / _polynomial(z - 11.5, ERF_IMPL_ID),
                0.5626493692,
            )
        } else if z < 24.0 {
            (
                _polynomial(z - 17.0, ERF_IMPL_JN) / _polynomial(z - 17.0, ERF_IMPL_JD),
                0.5634598136,
            )
        } else if z < 38.0 {
            (
                _polynomial(z - 24.0, ERF_IMPL_KN) / _polynomial(z - 24.0, ERF_IMPL_KD),
                0.5638477802,
            )
        } else if z < 60.0 {
            (
                _polynomial(z - 38.0, ERF_IMPL_LN) / _polynomial(z - 38.0, ERF_IMPL_LD),
                0.5640528202,
            )
        } else if z < 85.0 {
            (
                _polynomial(z - 60.0, ERF_IMPL_MN) / _polynomial(z - 60.0, ERF_IMPL_MD),
                0.5641309023,
            )
        } else {
            (
                _polynomial(z - 85.0, ERF_IMPL_NN) / _polynomial(z - 85.0, ERF_IMPL_ND),
                0.5641584396,
            )
        };
        let g = (-z * z).exp() / z;
...
fn erf_inv_impl(p: f64, q: f64, s: f64) -> f64 {
    let result = if p <= 0.5 {
        ...
        let g = p * (p + 10.0);
        let r = _polynomial(p, ERF_INV_IMPL_AN) / _polynomial(p, ERF_INV_IMPL_AD);
        g * y + g * r
    } else if q >= 0.25 {
        let y = 2.249481201171875;
        let g = (-2.0 * q.ln()).sqrt();
        let xs = q - 0.25;
        let r = _polynomial(xs, ERF_INV_IMPL_BN) / _polynomial(xs, ERF_INV_IMPL_BD);
        g / (y + r)
    } else {
        let x = (-q.ln()).sqrt();
        if x < 3.0 {
            ...
            let xs = x - 1.125;
            let r = _polynomial(xs, ERF_INV_IMPL_CN) / _polynomial(xs, ERF_INV_IMPL_CD);
            y * x + r * x
        } else if x < 6.0 {
            let y = 0.93995571136474609375;
            let xs = x - 3.0;
            let r = _polynomial(xs, ERF_INV_IMPL_DN) / _polynomial(xs, ERF_INV_IMPL_DD);
            y * x + r * x
        } else if x < 18.0 {
            let y = 0.98362827301025390625;
            let xs = x - 6.0;
            let r = _polynomial(xs, ERF_INV_IMPL_EN) / _polynomial(xs, ERF_INV_IMPL_ED);
            y * x + r * x
        } else if x < 44.0 {
            let y = 0.99714565277099609375;
            let xs = x - 18.0;
            let r = _polynomial(xs, ERF_INV_IMPL_FN) / _polynomial(xs, ERF_INV_IMPL_FD);
            y * x + r * x
        } else {
            let y = 0.99941349029541015625;
            let xs = x - 44.0;
            let r = _polynomial(xs, ERF_INV_IMPL_GN) / _polynomial(xs, ERF_INV_IMPL_GD);
            y * x + r * x
        }
    };
    s * result
